...
export async function fetchBlogListWithoutPagination(excludeId) {
    try {
      connectToDB();
      ...
      // Create a query to exclude a specific ID if provided.
      const blogQuery = excludeId
        ? Blog.find({ _id: { $ne: excludeId } })
        : Blog.find();
  
      const blog = await blogQuery.sort(sortOptions).exec();
  
      // Count the total number of blog that match the search criteria (without pagination).
      const totalBlogCount = await Blog.countDocuments();
  
      return { blog, totalBlogCount };
    } catch (error) {
      throw new Error(`${error.message}`);
    }
  }
