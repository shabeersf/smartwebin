...
async function uploadImage(image) {
  ...
  const imageFilename =
    currentDatetime.replace(/:/g, "-") + "_" + imageFile.name;
...
export async function fetchOtherList({
  searchString = "",
  pageNumber,
  pageSize,
  sortBy = "desc",
...
export async function updateOther({ id, title, cover_img, images }) {
  try {
    ...
    await Other.findOneAndUpdate(
      { _id: id },
      { title, cover_img: cover_imgFileName, images: imagesFileName }
    );
...
export async function fetchOtherListWithoutPagination() {
  try {
    connectToDB();
    ...
    const otherQuery = Other.find().sort(sortOptions);

    // Count the total number of other that match the search criteria (without pagination).
    const totalOtherCount = await Other.countDocuments();
...
export async function fetchPortfolioOther() {
  try {
    connectToDB();
    ...
    const otherQuery = Other.find().limit(6).sort(sortOptions);
...
export async function fetchPortfolioWebsite() {
  try {
    connectToDB();
    ...
    const portfolioQuery = Portfolio.find().limit(6).sort(sortOptions);

    // Count the total number of portfolio that match the search criteria (without pagination).
    const totalPortfolioCount = await Portfolio.countDocuments();
...
export async function fetchPortfolioBlog() {
  try {
    connectToDB();
    ...
    const blogQuery = Blog.find().limit(3).sort(sortOptions);
...
export async function fetchPortfolioPrint() {
  try {
    connectToDB();
    ...
    const printQuery = Print.find().limit(6).sort(sortOptions);

    // Count the total number of print that match the search criteria (without pagination).
    const totalPrintCount = await Print.countDocuments();
...
export async function fetchPortfolioMobile() {
  try {
    connectToDB();
    ...
    const mobileQuery = Mobile.find().limit(6).sort(sortOptions);

    // Count the total number of mobile that match the search criteria (without pagination).
    const totalMobileCount = await Mobile.countDocuments();
